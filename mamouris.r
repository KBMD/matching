# code from Mamouris, P., Nassiri, V., Molenberghs, G. et al. Fast and optimal algorithm for case-control matching using registry data: application on the antibiotics use of colorectal cancer patients. BMC Med Res Methodol 21, 62 (2021). https://doi.org/10.1186/s12874-021-01256-3
# This article is licensed under a Creative Commons Attribution 4.0 International License

# Code is from https://figshare.com/articles/journal_contribution/Additional_file_1_of_Fast_and_optimal_algorithm_for_case-control_matching_using_registry_data_application_on_the_antibiotics_use_of_colorectal_cancer_patients/14368925?file=27444263
# which for this file says: Licence CC BY + CC0 
# "Additional file 1. The functions of the ccoptimalmatch package."
...
# 1)	Function to calculate the iterations per round
...
iterations_per_round <- function (dataset, cluster_var, Id_Patient, total_cont_per_case, case_control, mat_per_case = NULL) {
    ...
    dup_con = one_to_one %>% distinct() %>% group_by((!!Id_Patient)) %>% filter(n() > 1) %>% arrange((!!Id_Patient), (!!total_cont_per_case)) %>% filter(row_number() ==
        1)
    if (nrow(dup_con) > 0) {
...
# 1)	Function to calculate the optimal matching
